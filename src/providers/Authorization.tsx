...
export function authorizationReducer(
  ...
  switch (action.type) {
    ...
    case 'failure':
      // ready: true covers the case where restoration failed on mount — we still
      // need to exit the Loading group, just with an error message attached.
...
export const AuthorizationProvider: FC<AuthorizationProviderProps> = ({ children }) => {
  ...
  // Restore authorization on mount. The auth token lives under its own key
  // inside services/authentication — it persists alongside the authorization
  // payload and needs no touch here.
  useEffect(() => {
    let cancelled = false;
    ...
    const restore = async () => {
      try {
        ...
        if (!cancelled) dispatch({ type: 'restore', response });
      } catch (error) {
        const message =
  ...
  const signUp = useCallback(
    async (payload: SignupPayload) => {
      ...
      try {
        ...
        // useAuthorization is the sole producer of the Basic-Auth token string.
        // It hands an opaque string to the authentication service; the httpClient
        // later prefixes `Basic ` when building the Authorization header.
  ...
  const logout = useCallback(async () => {
    try {
      // Wipes both halves of the persisted identity: the token (authentication
      // service) and the payload (authorization service). Same wipe regardless
      // of trigger — explicit user tap or a 401 from a protected endpoint.
      await clearAuthenticationToken();
      await clearAuthorizationStorage();
      dispatch({ type: 'reset' });
    } catch (error) {
      if (__DEV__) console.warn('Failed to clear session:', error);
